"""Complete database seeding script for Blood Bowl Manager.

Seeds all base catalog collections:
- base_rosters: Team templates from base_teams.json
- star_players: Star players from star_players.json
- perks: Skills and traits from skills.json
- skill_families: Skill categories from skills.json
"""
...
# ============================================================================
# SKILL FAMILY SEEDING
# ============================================================================


async def seed_skill_families(skills_data: dict) -> dict[str, dict]:
    """Seed skill_families collection and return family lookup."""
    families = skills_data.get("families", [])

    await SkillFamily.delete_all()
    print("Cleared existing skill_families")
    ...
    for fam in families:
        ...
        print(f"  Inserted family: {fam['name']['en']} ({fam['symbol']})")

    print(f"Seeded {len(families)} skill families\n")
    return family_lookup


# ============================================================================
# PERKS/SKILLS SEEDING
# ============================================================================


async def seed_perks(skills_data: dict) -> dict[str, dict]:
    """Seed perks collection and return perk lookup."""
    skills = skills_data.get("skills", [])

    await Perk.delete_all()
    print("Cleared existing perks")
    ...
    for skill in skills:
        ...
        # Build lookup for roster seeding
        family = skill.get("family", "general")
        family_to_symbol = {
            "agility": "A",
            "devious": "D",
        ...
        perk_lookup[skill["_id"]] = {
            "name": skill.get("name", {}).get("en", skill["_id"]),
            "category": family_to_symbol.get(family, "G"),
        }

    print(f"Seeded {len(skills)} perks/skills\n")
    return perk_lookup


# ============================================================================
# BASE ROSTERS SEEDING
# ============================================================================
...
def convert_team_to_roster(team_data: dict, perk_lookup: dict) -> BaseRoster:
    ...
    for char in team_data.get("characters", []):
        # Convert perks from string names to BasePerk objects
        char_perks = []
...
async def seed_base_rosters(teams_data: list, perk_lookup: dict):
    ...
    print("Cleared existing base_rosters")
    ...
    print(f"Seeded {len(teams_data)} base rosters\n")


# ============================================================================
# STAR PLAYERS SEEDING
# ============================================================================
...
async def seed_star_players(star_players_data: list):
    ...
    await StarPlayer.delete_all()
    print("Cleared existing star_players")

    for sp in star_players_data:
        star_player = convert_star_player(sp)
        await star_player.insert()
        teams_count = len(star_player.plays_for)
        print(f"  Inserted: {star_player.name} (plays for {teams_count} teams)")

    print(f"Seeded {len(star_players_data)} star players\n")
...
async def seed_database():
    """Complete database seeding from all JSON files."""

    print("=" * 60)
    print("Blood Bowl Manager - Database Seeding")
    print("=" * 60 + "\n")
    ...
    await init_beanie(
        ...
        document_models=[BaseRoster, StarPlayer, Perk, SkillFamily],
    )
    ...
    # Load base_teams.json
    print("Loading base_teams.json...")
    with open(
        os.path.join(base_dir, "config", "base_teams.json"), encoding="utf-8-sig"
    ) as f:
        teams_data = json.load(f)

    # Load star_players.json
    print("Loading star_players.json...")
    with open(
        os.path.join(base_dir, "config", "star_players.json"), encoding="utf-8-sig"
    ) as f:
        star_players_data = json.load(f)

    print(f"\nLoaded {len(skills_data.get('skills', []))} skills")
    print(f"Loaded {len(skills_data.get('families', []))} skill families")
    print(f"Loaded {len(teams_data)} teams")
    print(f"Loaded {len(star_players_data)} star players\n")

    # Seed in order (families and perks first for lookups)
    print("-" * 40)
    print("Seeding Skill Families...")
    print("-" * 40)
    family_lookup = await seed_skill_families(skills_data)

    print("-" * 40)
    print("Seeding Perks/Skills...")
    print("-" * 40)
    perk_lookup = await seed_perks(skills_data)

    print("-" * 40)
    print("Seeding Base Rosters...")
    print("-" * 40)
    await seed_base_rosters(teams_data, perk_lookup)

    print("-" * 40)
    print("Seeding Star Players...")
    print("-" * 40)
